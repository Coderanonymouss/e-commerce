...
const sortOptions = [{name: 'Price: Low to High', href: '#', current: false}, {
        ...
        <div>
                ...
                <div className="fixed inset-0 z-40 flex">
                    <DialogPanel
                        ...
                        <div className="flex items-center justify-between px-4">
                            <h2 className="text-lg font-medium text-gray-900">Сұрыптау</h2>
                            <button
                                type="button"
                        ...
                        <form className="mt-4 border-t border-gray-200">
                            {filters.map((section) => (<Disclosure key={section.id} as="div"
            ...
            <main className="mx-auto px-4 sm:px-6 lg:px-20">
                <div className="flex items-baseline justify-between border-b border-gray-200 pb-6 pt-24">
                    <h1 className="text-4xl font-bold tracking-tight text-gray-900">Тауар</h1>
                    <div className="flex items-center">
                        <Menu as="div" className="relative inline-block text-left">
                            ...
                            <MenuItems
                                ...
                                <div className="py-1">
                                    {sortOptions.map((option) => (<MenuItem key={option.name}>
                                        <a href={option.href}
                        ...
                        <button type="button" className="-m-2 ml-5 p-2 text-gray-400 hover:text-gray-500 sm:ml-7">
                            <span className="sr-only">Торды көру</span>
                        ...
                        <button type="button" onClick={() => setMobileFiltersOpen(true)} className="-m-2 ml-4 p-2 text-gray-400 hover:text-gray-500 sm:ml-6 lg:hidden">
                            <span className="sr-only">Cұрыптау</span>
                    ...
                    <div className="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-5">
                        <div>
                            <div className='py-5 flex justify-between items-center'>
                                <h1 className='text-lg opacity-50 font-bold'>Сұрыптау</h1>
                                <FilterListIcon/>
                            </div>
                                                ...
                                                <div className="space-y-4">
                                                    <FormControl>
                                                        <RadioGroup aria-labelledby="demo-radio-buttons-group-label"
                                                            ...
                                                            {section.options.map((option) => (<FormControlLabel
                                                                key={option.value}
                                                                value={option.id}
                                                                control={<Radio/>}
                                                                label={option.label}
                            ...
                            <div className="lg:col-span-4 w-full">
                                <div className='flex flex-wrap justify-center bg-white py-5'>
                                    {kurtaPage1.map((item) => <ProductCard key={item.id} product={item}/>)}
                                </div>
                            </div>
